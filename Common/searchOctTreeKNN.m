function [dists, nearestInds] = searchOctTreeKNN(point, knear, points, binParents, binCorners, pointBins)
...
% Last modified at 15.39 on 29/04/2025


% Function finds k nearest neighbours

[pointBin] = findPointBin(point, binParents, binCorners);
searchX = point(1); searchY = point(2); searchZ = point(3);
maxDist = inf; 
dists = []; nearestInds = [];
[maxDist, dists, nearestInds] = newNearCheck(searchX, searchY, searchZ, points, pointBins, maxDist, dists, nearestInds, pointBin, knear);
...
while counter <= length(checkBins)
    ...
    if dist < maxDist
        ...
        [maxDist, dists, nearestInds] = newNearCheck(searchX, searchY, searchZ, points, pointBins, maxDist, dists, nearestInds, checkBin, knear);
        end
    end
...
function [maxDist, dists, nearestInds] = newNearCheck(searchX, searchY, searchZ, points, pointBins, maxDist, dists, nearestInds, checkBin, knear)
    ...
    [newdists, newinds] = mink(newdists,knear); % Smallest distances to point in new bin
    newinds = binpointInds(newinds); % Closest points in new bin

    allInds = [nearestInds; newinds]; % Array of old and new inds

    [dists, minInds] = mink([dists; newdists],knear); % Finding minimum dists when considering old and new bin
    nearestInds = allInds(minInds); % Finding indexes of closest points when considering old and new bin
